...
import { formatDate, getStatusVariant, getStatusLabel, getRoomIcon } from '../utils/helpers';
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
          <div>
            <h1 className="text-4xl font-black text-gray-900 dark:text-white">My Bookings History</h1>
            <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Manage and view your academic space reservations.</p>
          </div>
          <Button variant="primary" icon="+" onClick={() => window.location.href = '/book'}>
            Book a Room
          </Button>
        </div>
                    ...
                    <tr key={booking._id} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-bold text-gray-900 dark:text-white">{booking.room_id?.name || 'N/A'}</p>
                            <p className="text-xs text-gray-500 dark:text-gray-400">{booking.room_id?.location || ''}</p>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300">{formatDate(booking.date)}</td>
                      <td className="px-6 py-4 text-sm text-gray-700 dark:text-gray-300 font-medium">{booking.start_time} - {booking.end_time}</td>
