...
const FacilityIssuesManagement = () => {
  ...
  const handleUpdateStatus = async (e) => {
    ...
    try {
      const response = await runMutationWithRefresh({
        mutate: () =>
          updateFacilityIssueStatus(selectedIssue._id, statusUpdateForm),
        refresh: [fetchIssues, fetchStats],
      });
  ...
  const handleDeleteIssue = async (issueId) => {
    ...
    try {
      const response = await runMutationWithRefresh({
        mutate: () => deleteFacilityIssue(issueId),
        refresh: [fetchIssues, fetchStats],
      });
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600">
            Monitor and manage reported facility issues
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  handleFilterChange("issue_type", e.target.value)
                }
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              >
                      ...
                      <tr key={issue._id} className="hover:bg-gray-50">
                        <td className="px-6 py-4">
                          ...
                          <div className="text-sm text-gray-500">
                            {issue.room_id?.room_code} -{" "}
                            {issue.room_id?.room_name}
                          </div>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`px-3 py-1 rounded-full text-xs font-semibold ${getSeverityColor(issue.severity)}`}
                          >
                            {issue.severity}
                          </span>
                        </td>
              ...
              <div className="space-y-6">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="space-y-1 text-sm text-gray-600">
                      ...
                      <p>
                        ...
                        {selectedIssue.room_id?.location}
                      </p>
                      <p>
                        <strong>Type:</strong>{" "}
                      ...
                      {selectedIssue.equipment_id && (
                        <p>
                          ...
                          {selectedIssue.equipment_id.name}
                        </p>
                      )}
                  ...
                  <div className="space-y-2">
                    <span
                      className={`inline-block px-4 py-2 rounded-lg text-sm font-medium border ${getStatusColor(selectedIssue.status)}`}
                    >
                      {selectedIssue.status.replace("_", " ")}
                    </span>
                    <span
                      className={`inline-block ml-2 px-4 py-2 rounded-lg text-sm font-semibold ${getSeverityColor(selectedIssue.severity)}`}
                    >
                    ...
                    <div className="text-sm text-gray-600">
                      <p>
                        ...
                        {selectedIssue.reported_by?.full_name ||
